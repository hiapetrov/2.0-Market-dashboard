    ...
    <div className="relative bg-gray-800 rounded-lg shadow-md mb-8 p-6 hover:shadow-lg transition-all border border-gray-700">
      ...
      <div className="flex justify-between items-center mb-6 pt-3">
        ...
        <div className="flex space-x-2">
          <button 
            className={`px-4 py-2 text-sm rounded-md ${timeRange === '7d' ? 'bg-indigo-700 text-white' : 'bg-gray-700 text-gray-300 border border-gray-600'}`}
            onClick={() => setTimeRange('7d')}
          >
            7 Days
          </button>
          <button 
            className={`px-4 py-2 text-sm rounded-md ${timeRange === '30d' ? 'bg-indigo-700 text-white' : 'bg-gray-700 text-gray-300 border border-gray-600'}`}
            onClick={() => setTimeRange('30d')}
          >
            30 Days
          </button>
          <button 
            className={`px-4 py-2 text-sm rounded-md ${timeRange === '90d' ? 'bg-indigo-700 text-white' : 'bg-gray-700 text-gray-300 border border-gray-600'}`}
